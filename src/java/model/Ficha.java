/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package model;

/**
 *
 * @author APRENDIZ
 */
public class Ficha {
    
    
    int CodFicha; 
    int CantidadAprendiz; 
    int Codprograma; 
    int Codaprendiz; 

    public Ficha() {
    }

    public int getCodFicha() {
        return CodFicha;
    }

    public void setCodFicha(int CodFicha) {
        this.CodFicha = CodFicha;
    }

    public int getCantidadAprendiz() {
        return CantidadAprendiz;
    }

    public void setCantidadAprendiz(int CantidadAprendiz) {
        this.CantidadAprendiz = CantidadAprendiz;
    }

    public int getCodprograma() {
        return Codprograma;
    }

    public void setCodprograma(int Codprograma) {
        this.Codprograma = Codprograma;
    }

    public int getCodaprendiz() {
        return Codaprendiz;
    }

    public void setCodaprendiz(int Codaprendiz) {
        this.Codaprendiz = Codaprendiz;
    }
    
    
    
}
